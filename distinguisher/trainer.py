...
class Trainer:
    def __init__(self, config_path: str):
        ...
        self.seed = self.config_manager.get('experiment.seed', random.randint(0, 2**31 - 1))
    ...
    def _setup_output_and_logging(self):
        ...
        self.logger.log(f"Random seed: {self.seed}")
    ...
    def _load_data(self):
        ...
        try:
            ...
            if memory_info_available:
                self.logger.log(f"  Current memory usage: {memory_mb:.2f} MB")
            else:
                self.logger.log(f"  Current memory usage: N/A")
            ...
            self.train_loader = self._preprocess_data_loader(self.train_loader, is_training=True)
            self.eval_loader = self._preprocess_data_loader(self.eval_loader, is_training=False)
    ...
    def _create_model(self):
        ...
        try:
            ...
            if hasattr(self.model, 'get_model_config'):
                logged_params = self.model.get_model_config()
            else:
                logged_params = params
            
            self.logger.log(f"  Model parameters: {logged_params}")
        except Exception as e:
