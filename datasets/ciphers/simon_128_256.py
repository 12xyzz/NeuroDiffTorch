...
class Simon_128_256_Dataset(Dataset):
    def __init__(self, n, nr, key_mode='random', key=None, diff=(0x0000000000000080,0x8000000000000000), neg='real_encryption', batch_size=10000):
        self.cipher = Simon_128_256()
    ...
    def generate_plaintext_triples(self, n, diff):
        p1_l = np.frombuffer(urandom(8*n), dtype=np.uint64)
        p1_r = np.frombuffer(urandom(8*n), dtype=np.uint64)

        p1_prime_l = p1_l ^ diff[0]
        p1_prime_r = p1_r ^ diff[1]
        
        p2_l = np.frombuffer(urandom(8*n), dtype=np.uint64)
        p2_r = np.frombuffer(urandom(8*n), dtype=np.uint64)
    ...
    def encrypt_plaintext_pairs(self, p1, p2, nr):
        if isinstance(p1, np.ndarray) and isinstance(p2, np.ndarray):
            ...
            n = p1.shape[0]
            if n == 0:
                return np.zeros((0, 2), dtype=np.uint64), np.zeros((0, 2), dtype=np.uint64)
            p1_l = p1[:, 0]
            p1_r = p1[:, 1]
            p2_l = p2[:, 0]
            p2_r = p2[:, 1]
            ...
            if n == 0:
                return [], []
            p1_l = np.array([p[0] for p in p1], dtype=np.uint64)
            p1_r = np.array([p[1] for p in p1], dtype=np.uint64)
            p2_l = np.array([p[0] for p in p2], dtype=np.uint64)
            p2_r = np.array([p[1] for p in p2], dtype=np.uint64)
    ...
    def generate_training_data(self, n, nr, diff=(0x0000000000000080,0x8000000000000000), neg='real_encryption', batch_size=10000):
    ...
    def _generate_keys(self, n, key_mode='random', key=None):
        if key_mode == 'random':
            single_key = None
            key_array = np.frombuffer(urandom(32*n), dtype=np.uint64).reshape(4, -1)
        elif key_mode == 'random_fixed':
            single_key = np.frombuffer(urandom(32), dtype=np.uint64)
            ...
            single_key = key
            key_array = np.array(key, dtype=np.uint64)
            if len(key_array) != 4:
                raise ValueError(f"Key must be 4 64-bit words for Simon128/256, got {len(key_array)}")
...
class Simon_128_256:
    def __init__(self, word_size=64, alpha=1, beta=8, gamma=2):
    ...
    def check_testvector(self):
        ...
        ks = self.expand_key(np.array(key, dtype=np.uint64).reshape(4, 1), 72)
        ct = self.encrypt(pt, ks)
        if ct == (0x8d2b5579afc8a3a0, 0x3bf72a87efe7b868):
