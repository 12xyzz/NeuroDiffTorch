...
class Xtea_Dataset(Dataset):
    ...
    def _generate_keys(self, n, key_mode='random', key=None):
        if key_mode == 'random':
            ...
            if len(key_array) != 4:
                raise ValueError(f"Key must be 4 32-bit words for XTEA, got {len(key_array)}")
...
class Xtea:
    ...
    def encrypt(self, p, k, r):
        ...
        s = 0
        for i in range(r):
            v0 = self.add_mod(v0, self.add_mod((v1 << 4) ^ (v1 >> 5), v1) ^ self.add_mod(s, k[s & 3]))
            s = self.add_mod(s, delta)
            v1 = self.add_mod(v1, self.add_mod((v0 << 4) ^ (v0 >> 5), v0) ^ self.add_mod(s, k[(s >> 11) & 3]))
        return (v0, v1)
    ...
    def decrypt(self, c, k, r):
        ...
        for i in range(r):
            v1 = (v1 - ((self.add_mod((v0 << 4) ^ (v0 >> 5), v0) ^ self.add_mod(s, k[(s >> 11) & 3])))) & self.mask_val
            s = (s - delta) & self.mask_val
            v0 = (v0 - ((self.add_mod((v1 << 4) ^ (v1 >> 5), v1) ^ self.add_mod(s, k[s & 3])))) & self.mask_val
        return (v0, v1)

    def check_testvector(self):
        # No unified standard test vectors for XTEA
        pass
