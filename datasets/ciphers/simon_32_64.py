...
class Simon_32_64_Dataset(Dataset):
    def __init__(self, n, nr, key_mode='random', key=None, diff=(0x0000,0x0400), neg='real_encryption', batch_size=10000):
        self.cipher = Simon_32_64()
    ...
    def generate_training_data(self, n, nr, diff=(0x0000,0x0400), neg='real_encryption', batch_size=10000):
    ...
    def _generate_keys(self, n, key_mode='random', key=None):
        if key_mode == 'random':
            ...
            if len(key_array) != 4:
                raise ValueError(f"Key must be 4 16-bit words for Simon32/64, got {len(key_array)}")
...
class Simon_32_64:
    def __init__(self, word_size=16, alpha=1, beta=8, gamma=2):
    ...
    def check_testvector(self):
        ...
        pt = (0x6565, 0x6877)
        ks = self.expand_key(np.array(key, dtype=np.uint16).reshape(4, 1), 32)
