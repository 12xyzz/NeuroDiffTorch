...
class Speck_128_256_Dataset(Dataset):
    ...
    def generate_plaintext_triples(self, n, diff):
        ...
        # Return as numpy arrays (n, 2) with dtype=np.uint64
    ...
    def _generate_keys(self, n, key_mode='random', key=None):
        if key_mode == 'random':
            ...
            if len(key_array) != 4:
                raise ValueError(f"Key must be 4 64-bit words for Speck128/256, got {len(key_array)}")
...
class Speck_128_256:
    def __init__(self, word_size=64, alpha=8, beta=3):
    ...
    def check_testvector(self):
        ...
        ks = self.expand_key(np.array(key, dtype=np.uint64).reshape(4, 1), 34)
